...
//___________Example 2___________________________
  ...
  var set = {-3, -1, 2, 4, 5, 7};
  set.add(12); // добавим число 12 в список
  set.addAll({4, 2, 6}); // добавим несколько чисел в список
  print(set.join(
      ", ")); // -3, -1, 2, 4, 5, 7, 6 выведем получившийся список в качестве разделитея запятая

  set.remove(4); // удалим число 4
  print(set.join(
      "; ")); // -3, -1, 2, 5, 7, 6 выведем элементы списка, разделитель точка с запятой

  set = set.where((element) => element > 2).toSet();
  print(set.join(", ")); // 5, 6, 7 получим те элементы которые больше 2

  set.clear();
  print('Пустой список: $set');
...
//___________Example 5___________________________
  var set1 = {1, 2, 3, 5};
